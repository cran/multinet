...
template <typename T>
T &&
boost_move_tt_declval() BOOST_NOEXCEPT;
...
class boost_move_tt_is_assignable
{
    ...
    template<class, class> static twochar
    test(...);
...
template <> struct unvoid_ref<void>
{
    typedef unvoid_ref & type;
};
template <> struct unvoid_ref<const void>
{
    typedef unvoid_ref & type;
};
template <> struct unvoid_ref<volatile void>
{
    typedef unvoid_ref & type;
};
...
//    is_integral
//////////////////////////
template<class T> struct is_integral_cv
{
    static const bool value = false;
};
template<> struct is_integral_cv<                     bool>
{
    static const bool value = true;
};
template<> struct is_integral_cv<                     char>
{
    static const bool value = true;
};
...
template<> struct is_integral_cv<                 char16_t>
{
    static const bool value = true;
};
#endif
#ifndef BOOST_NO_CXX11_CHAR32_T
template<> struct is_integral_cv<                 char32_t>
{
    static const bool value = true;
};
#endif
#ifndef BOOST_NO_INTRINSIC_WCHAR_T
template<> struct is_integral_cv<                  wchar_t>
{
    static const bool value = true;
};
#endif
template<> struct is_integral_cv<                    short>
{
    static const bool value = true;
};
...
template<> struct is_integral_cv<                      int>
{
    static const bool value = true;
};
...
template<> struct is_integral_cv<                     long>
{
    static const bool value = true;
};
...
struct is_reference_convertible_to_pointer
{
    ...
    test(U*);
    template <class U> static twochar
    test(...);
    static T&
    source();
...
struct empty_helper_t1 : public T
{
  ...
  private:
    ...
    empty_helper_t1&
    operator=(const empty_helper_t1&);
};
...
struct has_boost_move_no_copy_constructor_or_assign_type
{
    template <class U>
    static yes_type
    test(typename U::boost_move_no_copy_constructor_or_assign*);
...
struct is_copy_constructible
{
    ...
    template<class U> static typename add_reference<U>::type
    source();
    static no_type
    test(...);
    ...
    template <class U>
    static yes_type
    test(U&, bool_<sizeof(U(source<U>()))>* = 0);
#else
    template <class U>
    static yes_type
    test(U&, decltype(U(source<U>()))* = 0);
#endif
...
struct is_copy_assignable
{
    ...
    decltype(source<U&>() = source<const U&>(), yes_type() ) test(int);
    template <class>     static no_type
    test(...);
...
typedef
int (alignment_dummy::*member_ptr);
...
template <> struct aligned_struct<1>
{
    char data;
};
template <> struct aligned_struct<2>
{
    short data;
};
template <> struct aligned_struct<4>
{
    int data;
};
template <> struct aligned_struct<8>
{
    double data;
};
