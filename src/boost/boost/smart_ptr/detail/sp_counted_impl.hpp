...
void
sp_scalar_constructor_hook( void * px, std::size_t size, void * pn );
void
sp_scalar_destructor_hook( void * px, std::size_t size, void * pn );
...
template<class D> D *
get_local_deleter( D * /*p*/ ) BOOST_SP_NOEXCEPT
{
    return 0;
}

template<class D> D *
get_local_deleter( local_sp_deleter<D> * p ) BOOST_SP_NOEXCEPT;
...
template<class X> class BOOST_SYMBOL_VISIBLE sp_counted_impl_p: public sp_counted_base
{
  private:
    ...
    sp_counted_impl_p &
    operator= ( sp_counted_impl_p const & );
  ...
  public:
    ...
    void
    dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
    ...
    void *
    get_deleter( sp_typeinfo_ const & ) BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
        return 0;
    }

    void *
    get_local_deleter( sp_typeinfo_ const & ) BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
        return 0;
    }

    void *
    get_untyped_deleter() BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
        return 0;
    }
...
template<class P, class D> class BOOST_SYMBOL_VISIBLE sp_counted_impl_pd: public sp_counted_base
{
  private:
    ...
    sp_counted_impl_pd &
    operator= ( sp_counted_impl_pd const & );
  ...
  public:
    ...
    void *
    get_deleter( sp_typeinfo_ const & ti ) BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
        return ti == BOOST_SP_TYPEID_(D)? &reinterpret_cast<char&>( del ): 0;
    }

    void *
    get_local_deleter( sp_typeinfo_ const & ti ) BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
...
template<class P, class D, class A> class BOOST_SYMBOL_VISIBLE sp_counted_impl_pda: public sp_counted_base
{
  private:
    ...
    sp_counted_impl_pda &
    operator= ( sp_counted_impl_pda const & );
  ...
  public:
    ...
    void
    destroy() BOOST_SP_NOEXCEPT BOOST_OVERRIDE
    {
