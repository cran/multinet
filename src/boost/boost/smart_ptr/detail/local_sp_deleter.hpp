...
template<class D> class local_sp_deleter: public local_counted_impl_em
{
  ...
  public:
    ...
    explicit
  local_sp_deleter( D const& d ) BOOST_SP_NOEXCEPT:
    d_( d )
    {
    }
    ...
    explicit
  local_sp_deleter( D&& d ) BOOST_SP_NOEXCEPT:
    d_( std::move(d) )
    {
    }
    ...
    D&
    deleter() BOOST_SP_NOEXCEPT
    {
        return d_;
    }

    template<class Y> void
    operator()( Y* p ) BOOST_SP_NOEXCEPT
    {
        d_( p );
    }
    ...
    void
    operator()( boost::detail::sp_nullptr_t p ) BOOST_SP_NOEXCEPT
    {
        d_( p );
    }
