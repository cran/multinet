...
class match_results
{
  ...
  public:
...
#else
        :
        m_subs(), m_base(), m_null(), m_last_closed_paren(0), m_is_singular(true)
    {
        (void)a;
    }
#endif
    ...
    difference_type
    position(size_type sub = 0) const
    {
        if (m_is_singular)
        {
            raise_logic_error();
    ...
    string_type
    str(const char_type* sub) const
    {
        return (*this)[sub].str();
    }
    template <class Traits, class A>
    string_type
    str(const std::basic_string<char_type, Traits, A>& sub) const
    {
        return (*this)[sub].str();
    }
    template <class charT>
    string_type
    str(const charT* sub) const
    {
        return (*this)[sub].str();
    }
    template <class charT, class Traits, class A>
    string_type
    str(const std::basic_string<charT, Traits, A>& sub) const
    {
        return (*this)[sub].str();
    }
    const_reference
    operator[](int sub) const
    {
    ...
    //
    const_reference
    named_subexpression(const char_type* i, const char_type* j) const
    {
        //
    ...
    template <class charT>
    const_reference
    named_subexpression(const charT* i, const charT* j) const
    {
    ...
    int
    named_subexpression_index(const char_type* i, const char_type* j) const
    {
        //
        // Scan for the leftmost *matched* subexpression with the specified named.
    ...
    template <class charT>
    int
    named_subexpression_index(const charT* i, const charT* j) const
    {
        BOOST_STATIC_ASSERT(sizeof(charT) <= sizeof(char_type));
    ...
    const_reference
    operator[](const char_type* p) const
    {
        const char_type* e = p;
    ...
    string_type
    format(Functor fmt, match_flag_type flags = format_default) const
    {
        if (m_is_singular)
        {
            raise_logic_error();
    ...
    bool
    operator==(const match_results& that)const
    {
        if (m_is_singular)
        {
    ...
    const capture_sequence_type&
    captures(int i)const
    {
        if (m_is_singular)
        {
            raise_logic_error();
    ...
    void BOOST_REGEX_CALL
    set_second(BidiIterator i, size_type pos, bool m = true, bool escape_k = false)
    {
        if (pos)
        {
    ...
    void BOOST_REGEX_CALL
    set_size(size_type n, BidiIterator i, BidiIterator j)
    {
        value_type v(j);
    ...
    void BOOST_REGEX_CALL
    set_first(BidiIterator i, size_type pos, bool escape_k = false)
    {
    ...
    void BOOST_REGEX_CALL
    set_named_subs(boost::shared_ptr<named_sub_type> subs)
    {
        m_named_subs = subs;
    }
...
template <class BidiIterator, class Allocator>
void
swap(match_results<BidiIterator, Allocator>& a, match_results<BidiIterator, Allocator>& b)
{
    a.swap(b);
}
