...
struct has_boost_extensions_tag
{
    template <class U>
    static double
    checker(U*, typename U::boost_extensions_tag* = nullptr);
    static char
    checker(...);
    static T*
    get();
...
struct default_wrapper : public BaseT
{
    ...
    std::string
    error_string(::boost::regex_constants::error_type e)const
    {
    ...
    std::intmax_t
    toi(const char_type*& p1, const char_type* p2, int radix)const
    {
      ...
      >::type
{
  ...
  private:
    ...
    regex_traits_wrapper&
    operator=(const regex_traits_wrapper&);
};
