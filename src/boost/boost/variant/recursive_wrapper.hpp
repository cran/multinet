...
class recursive_wrapper
{
  ...
  public: // modifiers
    ...
    void
    swap(recursive_wrapper& operand) BOOST_NOEXCEPT
    {
        T* temp = operand.p_;
        operand.p_ = p_;
        p_ = temp;
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    recursive_wrapper&
    operator=(recursive_wrapper&& rhs) BOOST_NOEXCEPT
    {
        swap(rhs);
        return *this;
    }
...
template <typename T>
inline void
swap(recursive_wrapper<T>& lhs, recursive_wrapper<T>& rhs) BOOST_NOEXCEPT
{
    lhs.swap(rhs);
}
