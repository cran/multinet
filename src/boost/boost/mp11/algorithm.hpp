...
{
    template<class... W> static mp_identity<L<W...>>
            f( U*..., mp_identity<W>*... );
...
constexpr inline mp_index_holder
operator+( mp_index_holder const & v, bool f )
{
    if ( v.f_ )
    {
        return v;
...
cx_find_index( bool const * first, bool const * last )
{
    std::size_t m = 0;
...
template<class... T, class F> BOOST_MP11_CONSTEXPR F
mp_for_each_impl( mp_list<T...>, F && f )
{
    using A = int[sizeof...(T)];
    return (void)A{ ((void)f(T()), 0)... }, std::forward<F>(f);
}

template<class F> BOOST_MP11_CONSTEXPR F
mp_for_each_impl( mp_list<>, F && f )
{
...
template<class L, class F> BOOST_MP11_CONSTEXPR F
mp_for_each( F && f )
{
    return detail::mp_for_each_impl( mp_rename<L, mp_list>(), std::forward<F>(f) );
}
