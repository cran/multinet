...
class typeinfo
{
  ...
  public:
    ...
    bool
    before( typeinfo const& rhs ) const
    {
#if ( defined(_WIN32) || defined(__CYGWIN__) ) && ( defined(__GNUC__) || defined(__clang__) ) && !defined(BOOST_DISABLE_CURRENT_FUNCTION)
...
inline char const *
demangled_name( core::typeinfo const & ti )
{
    return ti.name();
}
...
BOOST_SYMBOL_VISIBLE inline void
core_typeid_lib_id()
{
}

template<class T> boost::core::typeinfo
core_typeid_< T >::ti_( core_typeid_< T >::name(), &core_typeid_lib_id );
