...
constexpr unsigned
cx_max( unsigned m1, unsigned m2 )
{
    return m1 > m2? m1: m2;
}
...
template<class T, unsigned Bm> struct update_modifiers
{
    template<class D> struct fn
    {
        ...
        static constexpr
        decltype(D::pointer) pointer = D::pointer;
        static constexpr
        decltype(D::name) name = D::name;
        static constexpr unsigned modifiers = ( mods & ~mod_any_access ) | access | mod_inherited | hidden;
    };
};

template<class T, unsigned Bm> template<class D> constexpr
decltype(D::pointer) update_modifiers<T, Bm>::fn<D>::pointer;
template<class T, unsigned Bm> template<class D> constexpr
decltype(D::name) update_modifiers<T, Bm>::fn<D>::name;
