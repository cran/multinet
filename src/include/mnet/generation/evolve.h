...
)
{
    ...
    {
        layer_idx[net->layers()->get_at_index(n)->name] = n;
    }
    ...
    for (size_t idx=0; idx<net->layers()->size(); idx++)
    {
        ...
        evolution_model[n]->init_step(net,net->layers()->get_at_index(n),available_actors[n]);
    ...
    for (long i=0; i<num_of_steps; i++)
    {
        for (size_t idx=0; idx<net->layers()->size(); idx++)
        {
            ...
            auto target_layer = net->layers()->get_at_index(n);
