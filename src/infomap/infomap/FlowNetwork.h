...
class FlowNetwork
{
  public:
    ...
    virtual void
    calculateFlow(const Network& network, const Config& config);
    ...
    const std::vector<double>&
    getNodeTeleportRates() const
    {
        return m_nodeTeleportRates;
    }
    const LinkVec&
    getFlowLinks() const
    {
  ...
  protected:
    ...
    void
    finalize(const Network& network, const Config& config, bool normalizeNodeFlow = false);
