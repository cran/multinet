...
class InfomapBase
{
  ...
  public:
    ...
    void
    run(Network& input, HierarchicalNetwork& output);

    void
    run(HierarchicalNetwork& output);

    bool
    initNetwork();

    bool
    initNetwork(Network& input);
    ...
    bool
    consolidateExternalClusterData(bool printResults = false);
    ...
    virtual void
    saveHierarchicalNetwork(HierarchicalNetwork& output, std::string rootName, bool includeLinks) = 0;

    virtual void
    debugPrintInfomapTerms() = 0;

  protected:

    virtual FlowDummy
    getNodeData(NodeBase& node) = 0;
    virtual std::vector<PhysData>&
    getPhysicalMembers(NodeBase& node) = 0;
    virtual StateNode&
    getMemoryNode(NodeBase& node) = 0;

    void
    initPreClustering(bool printResults = false);
    ...
    virtual void
    resetModuleFlowFromLeafNodes() = 0;
     ...
     */
    virtual void
    moveNodesToPredefinedModules() = 0;
    ...
    virtual unsigned int
    aggregateFlowValuesFromLeafToRoot() = 0;

    virtual double
    calcCodelengthOnAllNodesInTree() = 0;

    virtual double
    calcCodelengthOnRootOfLeafNodes(const NodeBase& parent) = 0;

    virtual double
    calcCodelengthOnModuleOfLeafNodes(const NodeBase& parent) = 0;

    virtual double
    calcCodelengthOnModuleOfModules(const NodeBase& parent) = 0;

    virtual std::pair<double, double>
    calcCodelength(const NodeBase& parent) = 0;

    virtual void
    generateNetworkFromChildren(NodeBase& parent) = 0;

    virtual void
    transformNodeFlowToEnterFlow(NodeBase* parent) = 0;

    virtual void
    cloneFlowData(const NodeBase& source, NodeBase& target) = 0;

    virtual void
    printNodeRanks(std::ostream& out) = 0;

    virtual void
    printFlowNetwork(std::ostream& out) = 0;

    virtual void
    sortTree(NodeBase& parent) = 0;
    ...
    numTopModules()
    {
        return m_treeData.root()->childDegree();
    }

    unsigned int
    numLeafNodes()
    {
        return m_treeData.numLeafNodes();
    }
    ...
    isTopLevel()
    {
        return (m_subLevel & (m_TOP_LEVEL_ADDITION-1)) == 0;
    }
    bool
    isSuperLevelOnTopLevel()
    {
        return m_subLevel == m_TOP_LEVEL_ADDITION;
    }
    bool
    isFullNetwork()
    {
        return m_subLevel == 0 && m_aggregationLevel == 0;
    }
    bool
    isFirstLoop()
    {
        return m_tuneIterationIndex == 0 && isFullNetwork();
    }
    bool
    haveModules()
    {
        return !m_treeData.root()->firstChild->isLeaf();
    }
    bool
    haveSubModules()
    {
        return haveModules() && !m_treeData.root()->firstChild->firstChild->isLeaf();
    }

    bool
    useHardPartitions()
    {
        return m_config.isMemoryNetwork() && m_config.hardPartitions && m_subLevel == 0;
    }

    unsigned int
    getLevelAggregationLimit()
    {
        return (m_config.fastFirstIteration && isFirstLoop()) ? 1 : m_config.levelAggregationLimit;
    }

    unsigned long int
    getSeedFromCodelength(double value)
    {
        return static_cast<unsigned long int>(value/m_config.minimumCodelengthImprovement);
    }

    void
    reseed(unsigned long int seed)
    {
  ...
  private:
    ...
    runPartition();
    double
    partitionAndQueueNextLevel(PartitionQueue& partitionQueue, bool tryIndexing = true);
    void
    tryIndexingIteratively();
    unsigned int
    findSuperModulesIterativelyFast(PartitionQueue& partitionQueue);
    unsigned int
    deleteSubLevels();
    void
    queueTopModules(PartitionQueue& partitionQueue);
    void
    queueLeafModules(PartitionQueue& partitionQueue);
    bool
    processPartitionQueue(PartitionQueue& queue, PartitionQueue& nextLevel, bool tryIndexing = true);
    void
    sortPartitionQueue(PartitionQueue& queue);
    void
    partition(unsigned int recursiveCount = 0, bool fast = false, bool forceConsolidation = true);
    void
    mergeAndConsolidateRepeatedly(bool forceConsolidation = false, bool fast = false);
    void
    generalTune(unsigned int level);
    void
    fineTune(bool leafLevel = true);
    void
    coarseTune(unsigned int recursiveCount = 0);
    /**
     ...
     */
    void
    partitionEachModule(unsigned int recursiveCount = 0, bool fast = false);
    void
    partitionEachModuleParallel(unsigned int recursiveCount = 0, bool fast = false);
    void
    initSubNetwork(NodeBase& parent, bool recalculateFlow = false);
    void
    initSuperNetwork(NodeBase& parent);
    void
    ...
    initMemoryNetwork();
    void
    initMemoryNetwork(MemNetwork& input);
    void
    initNodeNames(Network& network);
    bool
    checkAndConvertBinaryTree();
    void
    printNetworkData(std::string filename = "");
    void
    printNetworkData(HierarchicalNetwork& output, std::string filename = "");
    void
    printHierarchicalData(HierarchicalNetwork& hierarchicalNetwork, std::string filename = "");
    virtual void
    printClusterNumbers(std::ostream& out);
    void
    printTreeLevelSizes(std::ostream& out, std::string heading = "");
    unsigned int
    printPerLevelCodelength(std::ostream& out);
    void
    aggregatePerLevelCodelength(std::vector<PerLevelStat>& perLevelStat, unsigned int level = 0);
    void
    aggregatePerLevelCodelength(NodeBase& root, std::vector<PerLevelStat>& perLevelStat, unsigned int level);
    DepthStat
    calcMaxAndAverageDepth();
    void
    calcMaxAndAverageDepthHelper(NodeBase& root, unsigned int& maxDepth, double& sumLeafDepth,	unsigned int currentDepth);
...
struct TwoLevelCodelength
{
    ...
    codelength()
    {
        return indexLength + moduleLength;
    }
    double indexLength;
    double moduleLength;
};
...
struct PerLevelStat
{
    ...
    codelength()
    {
        return indexLength + leafLength;
    }
    unsigned int
    numNodes()
    {
        return numModules + numLeafNodes;
    }
...
{
    bool
    operator()(const PerIterationStats& a, const PerIterationStats& b)
    {
...
{
    unsigned int
    operator()(double result, const PerIterationStats& s)
    {
...
struct AddMapValues
{
    ...
    Value
    operator()(Value value, const Pair& pair) const
    {
