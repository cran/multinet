...
{
    bool
    operator() (const ChildEdge& lhs, const ChildEdge& rhs) const
    {
...
class SNode
{
  public:
    ...
    SerialTypes::edgeSize_t
    numSerializableChildEdges() const
    {
        return SerialTypes::numeric_cast<SerialTypes::edgeSize_t>(childEdges.size());
    }

    std::string
    printState(unsigned int indexOffset = 0)
    {
  ...
  public:
    ...
    // Capacity:
    unsigned int
    serializationSize(bool writeEdges)
    {
    ...
    // Operations:
    void
    serialize(SafeBinaryOutFile& outFile, SerialTypes::childPos_t childPosition, bool writeEdges)
    {
        using namespace SerialTypes;
    ...
    unsigned short
    deserialize(SafeBinaryInFile& dataStream)
    {
        using namespace SerialTypes;
    ...
    unsigned short
    deserializeEdges(SafeBinaryInFile& dataStream, bool directedEdges)
    {
        using namespace SerialTypes;
        edgeSize_t numEdges;
        dataStream >> numEdges;
     ...
     */
    bool
    createChildEdge(SerialTypes::edgeSize_t sourceIndex, SerialTypes::edgeSize_t targetIndex, double flow, bool directed)
    {
...
class LeafIterator
{
  public:
    ...
    LeafIterator &
    operator= (const LeafIterator& other)
    {
        m_root = other.m_root;
        m_current = other.m_current;
        m_depth = other.m_depth;
    ...
    bool
    operator==(const LeafIterator& rhs) const
    {
...
class TreeIterator
{
  public:
    ...
    TreeIterator &
    operator= (const TreeIterator& other)
    {
        m_root = other.m_root;
        m_current = other.m_current;
        m_depth = other.m_depth;
...
tryNext:
    ...
    bool
    operator==(const TreeIterator& rhs) const
    {
        return m_current == rhs.m_current;
    }
...
class ChildIterator
{
  public:
    ...
    ChildIterator &
    operator= (const ChildIterator& other)
    {
        m_root = other.m_root;
        m_current = other.m_current;
        m_childIndex = other.m_childIndex;
    ...
    bool
    operator==(const ChildIterator& rhs) const
    {
        return m_current == rhs.m_current;
    }
...
class HierarchicalNetwork
{
  public:
    ...
    void
    init(std::string networkName, double codelength, double oneLevelCodelength);

    void
    clear();

    void
    clear(const Config& conf);
    ...
    numTopModules()
    {
        return m_rootNode.childDegree();
    }

    LeafIterator
    leafIter(int moduleIndexDepth = -1)
    {
        return LeafIterator(&m_rootNode, moduleIndexDepth);
    }

    TreeIterator
    treeIter(int moduleIndexDepth = -1)
    {
        return TreeIterator(&m_rootNode, moduleIndexDepth);
    }

    SNode&
    addNode(SNode& parent, double flow, double exitFlow);

    SNode&
    addLeafNode(SNode& parent, double flow, double exitFlow, const std::string& name, unsigned int leafIndex);
    SNode&
    addLeafNode(SNode& parent, double flow, double exitFlow, const std::string& name, unsigned int leafIndex, unsigned int originalIndex);
    SNode&
    addLeafNode(SNode& parent, double flow, double exitFlow, const std::string& name, unsigned int leafIndex,
                unsigned int originalIndex, bool isMemoryNode, unsigned int stateIndex, unsigned int physIndex);

    void
    prepareAddLeafNodes(unsigned int numLeafNodes);
     ...
     */
    bool
    addLeafEdge(unsigned int sourceLeafNodeIndex, unsigned int targetLeafNodeIndex, double flow);

    void
    propagateNodeNameUpInHierarchy(SNode& node);
     ...
     */
    void
    writeStreamableTree(const std::string& fileName, bool writeEdges);

    void
    readStreamableTree(const std::string& fileName);

    void
    writeHumanReadableTree(const std::string& fileName, bool writeHierarchicalNetworkEdges = false);
     ...
     */
    void
    writeClu(const std::string& fileName, int moduleIndexDepth = 1);

    void
    readHumanReadableTree(const std::string& fileName);

    void
    writeMap(const std::string& fileName);
  ...
  private:
    ...
    void
    writeHumanReadableTreeRecursiveHelper(std::ostream& out, SNode& node, std::string prefix = "");
    void
    writeHumanReadableTreeFlowLinksRecursiveHelper(std::ostream& out, SNode& node, std::string prefix = "");

    static bool
    compareLeafNodePredicate(const SNode* lhs, const SNode* rhs)
    {
        return (lhs->originalLeafIndex < rhs->originalLeafIndex);
    }
