            ...
            RSUPP minsupp, RSUPP maxsupp)
{
    ...
    if (!psp)
    {
        return NULL;    /* create the base structure */
    }
...
psp_delete (PATSPEC *psp)
{
    ...
    if (psp->rows)                /* if there are pattern spectrum rows */
    {
        ...
        for (size = psp->minsize; size < psp->max; size++)
            if (psp->rows[size].frqs)
            {
                free(psp->rows[size].frqs);
            }
...
static int
resize (PATSPEC *psp, ITEM size, RSUPP supp)
{
    ...
    if (size > psp->max)          /* if outside of size range */
    {
        ...
        if (n > (size_t)psp->maxsize)
        {
            n = (size_t)psp->maxsize;
        }
        ...
        if (!row)
        {
            return psp->err = -1;    /* enlarge the row array */
        }
    ...
    else if (supp > row->min)
    {
        min = row->min;
    }

    else if (supp > row->min -BLKSIZE)
    {
        min = row->min -BLKSIZE;
    }
    ...
    else if (supp < row->max)
    {
        max = row->max;
    }

    else if (supp < row->max +BLKSIZE)
    {
        max = row->max +BLKSIZE;
    }
    ...
    if (size <= 0)
    {
        min = max = supp;    /* only one counter for size = 0 */
    }
    ...
    if (!p)
    {
        return psp->err = -1;    /* enlarge the counter array */
    }
...
int
psp_setfrq (PATSPEC *psp, ITEM size, RSUPP supp, size_t frq)
{
    /* --- set a counter value */
    PSPROW *row;                  /* to access the table row */
...
int
psp_incfrq (PATSPEC *psp, ITEM size, RSUPP supp, size_t frq)
{
...
psp_addpsp (PATSPEC *dst, PATSPEC *src)
{
    ...
    for (size = src->minsize; size <= src->max; size++)
    {
        ...
        if (!row->frqs)
        {
            continue;    /* if no counters exist, skip row */
        }
...
choose (ITEM n, ITEM k)
{
    ...
    if (n-k < k)
    {
        k = n-k;    /* minimize the loop executions */
    }
...
static double
samplelp (double *probs, ITEM n, ITEM k)
{
    ...
    if (k <= 0)
    {
        return 0.0;    /* check for an empty sample */
    }

    if (k >  n)
    {
        k = n;    /* ensure that sampling is possible */
    }
    ...
    for (i = 0; i < 8; i++)       /* draw 8/16 samples (permutations) */
    {
        if (i <= 0)
        {
            dbl_qsort  (probs, (size_t)k, +1);
        }

        else
        {
            dbl_shuffle(probs, (size_t)k, drand);
        }
...
taslots (TABAG *tabag)
{
    ...
    if (!cnts)
    {
        return NULL;    /* create a slot counter array */
    }
    ...
    while (--n > 0)               /* find the last non-zero entry */
        if (cnts[n] > 0)
        {
            break;    /* (size of largest transaction) */
        }
...
static void
siftx (HEAPELEM *heap, ITEM lft, ITEM rgt)
{
...
static size_t*
slots_perm (TRAINSET *tns, double width)
{
    /* --- count slots per item set size */
    ITEM     i, k, n;             /* loop variables, number of trains */
    ...
    while (--n > 0)               /* shrink to the counter array and */
        if (cnts[n] > 0)
        {
            break;    /* find the last non-zero entry */
        }
...
static void
sift (const double *heap[], ITEM lft, ITEM rgt)
{
    /* --- let element sift down in heap */
    ITEM  i;                      /* index of first successor in heap */
...
static size_t*
slots_seqs (TRAINSET *tns, double width)
{
    /* --- count slots per item set size */
    ITEM   i, k, n;               /* loop variables, number of trains */
    ...
    if (!pp)
    {
        free(cnts);    /* point process and heap */
        return NULL;
    }
...
static int
simpest (PATSPEC *psp, size_t *cnts, ITEM n,
         RSUPP smax, size_t equiv, int seq)
{
    ...
    for (z = psp->minsize; z < n; z++)
    {
        if (cnts[z] <= 0)
        {
            continue;    /* if there are no slots, skip size */
        }
...
static int
cplxest (PATSPEC *psp, size_t *cnts, double *probs, ITEM n,
    ...
    for (z = psp->minsize; z < n; z++)   /* traverse pattern sizes */
    {
        ...
        for (i = 0; i < smpls; i++) /* draw a certain number of samples */
        {
            ...
            for (c = 1; c <= smax; c++)
            {
                ...
                {
                    break;    /* compute next distribution value */
                }
            }                         /* update probability distribution */

            if (c > s)
            {
                s = c;    /* update maximum coincidence count */
            }
        ...
        for (c = psp->minsupp; c < s; c++)
        {
            ...
            if (frq <= 0)
            {
                continue;    /* compute equivalent frequency */
            }
...
int
psp_tbgest (TABAG *tabag, PATSPEC *psp, size_t equiv,
            double alpha, size_t smpls)
{
    ...
    if (!cnts)
    {
        return -1;    /* (evaluate transaction sizes) */
    }
    ...
    if (alpha <= 0)               /* check for equal probabilities */
    {
        ...
        free(cnts);
        return r;       /* execute simple estimation */
    ...
    if (!probs)
    {
        free(cnts);    /* allocate work memory */
        return -1;
    }
    ...
    free(probs);
    free(cnts);      /* delete the working memory */
...
int
psp_tnsest (TRAINSET *tns, PATSPEC *psp, size_t equiv,
            double width, double alpha, size_t smpls, int target)
{
    ...
    if (!cnts)
    {
        return -1;    /* (evaluate points in trains) */
    }
...
psp_report (PATSPEC *psp, TABWRITE *twr, double scale)
{
    ...
    for (size = psp->minsize; size <= psp->max; size++)
    {
        ...
        for (supp = row->min; supp <= row->max; supp++)
        {
            ...
            twr_puts(twr, buf);
            twr_fldsep(twr);
            twr_printf(twr, "%"RSUPP_FMT, supp);
            twr_fldsep(twr);
            twr_printf(twr, "%g", scale*(double)frq);
            twr_recsep(twr);
        ...
        twr_printf(twr, "%"ITEM_FMT, size);
        twr_fldsep(twr);
        twr_printf(twr, "%"RSUPP_FMT, row->min);
        twr_fldsep(twr);
        twr_printf(twr, "%"RSUPP_FMT, row->max);
        twr_recsep(twr);
...
psp_show (PATSPEC *psp)
{
    ...
    if (!psp->rows)
    {
        return;    /* if there is no row array, abort */
    }
...
int main (int argc, char *argv[])
{
    ...
    if (!psp)
    {
        error(E_NOMEM);    /* create a pattern spectrum */
    }
    ...
    if (!twrite)
    {
        error(E_NOMEM);    /* configure the characters */
    }
