...
struct StateNode
{
    ...
    {}
    StateNode&
    operator=(const StateNode& other)
    {
    ...
    void
    subtractIndexOffset(unsigned int indexOffset)
    {
    ...
    bool
    operator<(StateNode other) const
    {
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const StateNode& node)
    {
        return out << "(" << node.stateIndex << "-" << node.physIndex << ")";
    }

    std::string
    print(unsigned int indexOffset = 0) const
    {
    ...
    std::string
    print(const std::vector<std::string>& names, unsigned int indexOffset = 0) const
    {
        std::ostringstream out;
...
class NodeBase
{
  public:
    ...
    const_pre_depth_first_iterator
    begin_depthFirst() const
    {
        return const_pre_depth_first_iterator(this);
    }

    const_pre_depth_first_iterator
    end_depthFirst() const
    {
    ...
    const SubStructure&
    getSubStructure() const
    {
        return m_subStructure;
    }

    const NodeBase*
    getSubInfomapRoot() const;
    ...
    bool
    operator ==(const NodeBase& rhs) const
    {
        return this == &rhs;
    }
    //	{ return id == rhs.id; }
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const NodeBase& node)
    {
    ...
    void
    setChildDegree(unsigned int value);

    void
    setNumLeafNodes(unsigned int value);
    ...
    void
    remove(bool removeChildren);

    void
    deleteChildren();

    EdgeType*
    addOutEdge(NodeBase& target, double weight, double flow = 0.0)
    {
...
inline
void
NodeBase::remove(bool removeChildren)
{
...
inline
unsigned int
NodeBase::replaceChildrenWithGrandChildren()
{
    if (firstChild == 0)
    {
        return 0;
...
inline
void
NodeBase::replaceChildrenWithGrandChildrenDebug()
{
    if (firstChild == 0)
    {
        return;
...
inline
void
NodeBase::replaceWithChildrenDebug()
{
    if (isLeaf() || isRoot())
    {
        return;
...
inline
void
NodeBase::setChildDegree(unsigned int value)
{
    m_childDegree = value;
...
inline
void
NodeBase::setNumLeafNodes(unsigned int value)
{
...
class Node : public NodeBase
{
  public:
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const node_type& node)
    {
        //		return out << "n" << node.id << " (" << node.data << ")";
